...
ggplot(data = data_long, aes(x = Qualityclass, y = Value, fill = Qualityclass)) +
  ...
  theme_minimal() +
  labs(title = "Boxplots of 4 variables by quality",
       x = "qualityclass",
       y = "Value") +
...
```
aroma flavor acidity altitude_mean_meters are both significant these varibales will be saved and AIC decreased to min.
```{r}
library(broom)
library(knitr)
library(dplyr)
...
$$\ln\left(\frac{p}{1-p}\right) = \alpha + \beta_1 \cdot \textrm{aroma} +
    \beta_2 \cdot \textrm{flavor}+
    \beta_3 \cdot \textrm{acidity}+ 
    \beta_4 \cdot \textrm{altitude_mean_meters}$$
